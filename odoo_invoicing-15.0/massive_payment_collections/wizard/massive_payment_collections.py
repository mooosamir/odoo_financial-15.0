...
class MassivePaymentCollections(models.TransientModel):
    ...
    def _get_batch_available_partner_banks(self, batch_result, journal):
        ...
        if payment_values['payment_type'] == 'inbound':
            ...
            return batch_result['lines'].partner_id.bank_ids.filtered(
                lambda x: x.company_id.id in (False, company.id))._origin
        ...
        for wizard in self:
            if wizard.can_edit_wizard:
                ...
                if wizard.payment_method_line_id.name == 'Detracción':
                    invoice_date = batches[0]['lines'].move_id.invoice_date
                    rate_id = self.env['res.currency.rate'].search([('name', '=', invoice_date)])
                    wizard.to_force_exchange_rate = rate_id[0].company_rate if rate_id else 0
                else:    
                    wizard.to_force_exchange_rate = batches[0]['lines'].move_id.to_force_exchange_rate
            else:
    ...
    def _create_payment_vals_from_batch(self, batch_result):
        ...
        return {
            ...
            'to_force_exchange_rate': batch_result['lines'][0].move_id.to_force_exchange_rate,
            'payment_method_line_id': self.payment_method_line_id.id,
